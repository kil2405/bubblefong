...
/*Table structure for table `mini_game_play_log_tb` */

DROP TABLE IF EXISTS `mini_game_play_log_tb`;

CREATE TABLE `mini_game_play_log_tb` (
  ...
  `room_uid` VARCHAR(48) NOT NULL COMMENT '미니게임 룸 UID',
  `user_id` integer(11) NOT NULL COMMENT '유저 고유아이디',
  `game_id` integer(11) NOT NULL COMMENT '미니게임 아이디',
  `rank` integer(11) NOT NULL COMMENT '미니게임 랭킹',
  `ruby` integer(11) NOT NULL COMMENT '획득 루비',
  `is_ticket` tinyint(4) NOT NULL COMMENT '티켓 사용여부',
  `reg_date` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '최초 생성 시간',
  PRIMARY KEY (`seq`, `ptn_month`, `ptn_day`, `log_date`, `room_uid`, `user_id`)
