...
/*Table structure for table `admin_character_modify_log_tb` */

DROP TABLE IF EXISTS `admin_character_modify_log_tb`;

CREATE TABLE `admin_character_modify_log_tb` (
  ...
  `name` VARCHAR(128) DEFAULT NULL COMMENT '캐릭터 명',
  `grade` TINYINT(4) NOT NULL COMMENT '캐릭터 등급',
  `character_level` TINYINT(4) NOT NULL COMMENT '캐릭터 레벨',
  `character_upgrade` TINYINT(4) NOT NULL COMMENT '캐릭터 레벨',
  `daily_earn` INT(11) NOT NULL COMMENT '현재 BP',
