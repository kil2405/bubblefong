...
/*Table structure for table `error_log_tb` */

DROP TABLE IF EXISTS `error_log_tb`;

CREATE TABLE `error_log_tb` (
  `error_num` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '에러 번호',
  ...
  `error_code` int(11) DEFAULT NULL COMMENT '에러코드',
  `request_body` varchar(4096) DEFAULT NULL COMMENT 'body',
  `error_text` varchar(8192) DEFAULT NULL COMMENT '에러내용',
  `reg_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '최초 생성 시간',
  PRIMARY KEY (`error_num`,`ptn_month`)
